...
fn test_contributor_serialization() {
    let contributor = Contributor {
        wallet_address: contract_address_const::<0x1234>(), amount: 100, timestamp: 1_700_000_000,
    };
...
fn test_contributor_edge_case() {
    let contributor = Contributor {
        wallet_address: contract_address_const::<0x5678>(), amount: 0, timestamp: 1_700_000_000,
    };
...
use graso_contract::interfaces::irealestateido::{
    IRealEstateIDODispatcher, IRealEstateIDODispatcherTrait,
};
use snforge_std::{
...
fn test_get_property_info_returns_default_for_nonexistent() {
    ...
    assert!(
        retrieved_property.current_amount == 0,
        "Should return zero current_amount for nonexistent property",
    );
}
